...
def get_loaders(train_dir, test_dir, batch_size, tokenizer, max_token_len, num_workers, training=True):
...
def eval_fn(loader, model, criterion, metric_collection, device):
    ...
    accuracy = metric_collection['MulticlassAccuracy'].compute() * 100

    print(f"Got on test set --> Accuracy: {accuracy:.3f}, Loss: {loss:.3f}")
